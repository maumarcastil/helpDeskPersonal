...
1. **Read the ticket.** Call \`get_ticket({ ticketId })\`. It must have
   \`triage\` data and be \`InProgress\` or \`PendingUserConfirmation\`; if not,
   see [Failure handling](#failure-handling).
2. **Run the connectivity-diagnostic skill.** It runs \`run_diagnostic\`,
...
3. **Hand off.** When the skill's procedure ends in escalation, hand off to
