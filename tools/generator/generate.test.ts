...
describe("writeRenderedFiles / checkRenderedFiles against a temp output root", () => {
  ...
  it("cleans up the sibling temp file and rethrows when a later rename fails (e.g. a directory at the target path)", async () => {
    // .mcp.json's target path is occupied by a directory, so writeFile to
    // the temp path succeeds but rename onto the real path fails.
    await mkdir(join(outRoot, ".mcp.json"), { recursive: true });

    await expect(writeRenderedFiles(files, outRoot)).rejects.toThrow();

    const rootEntries = await readdir(outRoot);
    const leftoverTempFiles = rootEntries.filter((entry) => entry.includes(".tmp-"));
    expect(leftoverTempFiles).toEqual([]);
  });

  it("write mode prunes a stale generated file whose definition was removed, but never deletes a hand-written unmarked file", async () => {
    await writeRenderedFiles(files, outRoot);

    // Simulates a file generated by a previous run whose definition was
    // since removed: it carries the marker but is not part of this run's
    // expected output.
    await writeFile(
      join(outRoot, ".claude/agents", "removed-agent.md"),
      `---\nname: removed-agent\n---\n${GENERATED_FILE_MARKER}\nstale\n`,
      "utf8",
    );
    // A hand-written file with no marker must never be deleted by pruning.
    await writeFile(join(outRoot, ".claude/agents", "hand-written.md"), "not generated\n", "utf8");

    await writeRenderedFiles(files, outRoot);

    const entries = (await readdir(join(outRoot, ".claude/agents"))).sort();
    expect(entries).toEqual(["hand-written.md", "triage.md"]);
  });

  it("checkRenderedFiles reports a leftover file in a fully vacated owned directory as extra", async () => {
    await writeRenderedFiles(files, outRoot);
    await writeFile(join(outRoot, ".claude/agents", "stale.md"), "stale\n", "utf8");

    // Render this pass expects nothing at all in .claude/agents (the
    // directory was fully vacated), so only .mcp.json is passed here.
    const result = await checkRenderedFiles([{ path: ".mcp.json", contents: '{"a":1}\n' }], outRoot);

    expect(result.ok).toBe(false);
    expect(result.differences).toContainEqual({ path: ".claude/agents/stale.md", reason: "extra" });
    expect(result.differences).toContainEqual({ path: ".claude/agents/triage.md", reason: "extra" });
  });
});
